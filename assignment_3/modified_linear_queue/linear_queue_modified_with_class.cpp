...
public:
    ModifiedLinearQueue(int queueSize) : front(-1), rear(-1), size(queueSize) {
        if (size <= 0) {
...
int main() {
    ...
    ModifiedLinearQueue queue(queueSize);

    while (1) {
        cout << endl << "=== Modified Linear Queue Operations ===" << endl;
        cout << "1. Enqueue (Insert)" << endl;
        cout << "2. Dequeue (Remove with left shift)" << endl;
        cout << "3. Display" << endl;
        cout << "4. Peek" << endl;
        cout << "5. Exit" << endl;
        cout << "Selection: ";
